...
class SwitchgearCustomerInsightTests(unittest.TestCase):
    ...
    def test_chint_pain_opportunities_include_panel_ka_playbook(self):
        ...
        self.assertEqual(len(summary["key_analysis"]), 3)
        key_analysis = {item["audience"]: item for item in summary["key_analysis"]}
        self.assertIn("盘厂客户经理", key_analysis)
        self.assertIn("销售", key_analysis)
        self.assertIn("战略部门", key_analysis)
        self.assertIn("竞合型盘厂KA", key_analysis["盘厂客户经理"]["title"])
        self.assertIn("CRM", key_analysis["盘厂客户经理"]["what_to_do"])
        self.assertIn("BOM", key_analysis["销售"]["what_to_do"])
        self.assertIn("业主/设计院/EPC前置规格", key_analysis["销售"]["analysis"])
        self.assertIn("国产低压龙头", key_analysis["战略部门"]["title"])
        self.assertIn("生命周期TCO", key_analysis["战略部门"]["risk_or_watch"])
        self.assertIn("项目入口", summary["substitution_chain"][1]["step"])
        self.assertIn("FAT/SAT", summary["substitution_chain"][4]["insight"])
        first_takeaway = summary["module_takeaways"][0]
        self.assertIn("manager_focus", first_takeaway)
        self.assertIn("sales_focus", first_takeaway)
        self.assertIn("strategy_focus", first_takeaway)
        self.assertTrue(first_takeaway["manager_focus"].startswith("看点在"))
        self.assertTrue(first_takeaway["sales_focus"].startswith("看点在"))
        self.assertTrue(first_takeaway["strategy_focus"].startswith("看点在"))
        self.assertIn("主体关系边界", first_takeaway["manager_focus"])
        self.assertIn("项目角色识别", first_takeaway["sales_focus"])
...
"""
    ...
    def test_dashboard_preserves_full_excel_framework(self):
        ...
        self.assertIn("主体口径", summary["module_takeaways"][0]["manager_focus"])
        self.assertIn("设计院", summary["module_takeaways"][1]["sales_focus"])
        self.assertIn("授权型项目盘厂", summary["module_takeaways"][0]["strategy_focus"])
    ...
    def test_shenghong_dashboard_uses_switchgear_ka_summary(self):
        ...
        self.assertIn("主体与基地拆分", summary["module_takeaways"][0]["manager_focus"])
        self.assertIn("MCC", summary["module_takeaways"][1]["sales_focus"])
        self.assertIn("连续生产", summary["module_takeaways"][0]["strategy_focus"])
